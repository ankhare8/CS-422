...
public class ShoppingList implements PrintList, Cloneable{
	...
	/**Method to add an item by entering its name, quantity and priority to the ShoppingList
	...
	public boolean add(String itemName, String quantity, String priority) {
		if (!search(itemName)) {
			int intP = Integer.parseInt(priority);
	...
	/**Method to add an item to the ShoppingList by passing in an object Item
	 * @param Item the item*/ 
...
//				e.printStackTrace();
	...
	private int parition(ArrayList<Item> currList, int left, int right) {
		...
		int pivot = Integer.parseInt(currList.get((left+right)/2).getPriority());
		...
		while(biggestIndex <= smallestIndex) {
			...
			while(Integer.parseInt(currList.get(biggestIndex).getPriority()) > pivot) {
				biggestIndex++;
			}
			...
			while(Integer.parseInt(currList.get(smallestIndex).getPriority()) < pivot) {
				smallestIndex--;
			}
