...
public class Budget implements PrintList{
	...
	public void shop(){
		...
		for(int i = SL.size() - 1; i >= 0; i--) {
			...
			int intP = Integer.parseInt(item.getPriority());
			int intQ = Integer.parseInt(item.getQuantity());
			int cost = item.getPriceCents();
			
			int quantityPurchased = 0;

			//calculate the total cost of buying the entire intQ of the item at its price.
			int totalCost = cost * intQ;

			//if the budget is greater than or equal to the total cost, buy all of the item's intQ, 
			...
			//buy as much of the item as possible while the budget is not exhausted
				...
				//and updating the intQ of the item purchased accordingly
				while (intQ != 0 && budgetCents >= cost) {
					budgetCents -= cost;  
					quantityPurchased++;
					intQ--;
				}
				item.setQuantity(intQ + "");
			}
			
			//if you ended up purchasing something, create a new item with the same name, intP, 
			//and price as the original item but with the intQ set to the intQ purchased, 
			//and add it to the purchased list.
			if (quantityPurchased != 0) {
				Item pItem = new Item(name, intP + "", quantityPurchased + "", cost);
				purchased.add(pItem);
			}
		}
	}
